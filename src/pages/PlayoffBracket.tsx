...
export function PlayoffBracket() {
  ...
  if (!bracket) {
    return (
      <div className="bg-gray-50 rounded-2xl p-16 text-center space-y-3">
        ...
        <Link to={`/leagues/${leagueId}`} className="inline-block text-sm font-semibold text-green-700 hover:text-green-900">
          ← Back to Dashboard
        </Link>
      </div>
    );
  ...
  if (bracket.rounds.length === 0) {
    ...
    return (
      <div className="space-y-6">
        <PageHeader config={config} leagueId={leagueId!} badge="projected" />
  ...
  return (
    <div className="space-y-6">
      <PageHeader config={config} leagueId={leagueId!} badge={config.status} />
  ...
  return (
    <div>
      ...
      <div className="flex items-start justify-between gap-4 mt-1">
        ...
        <Link to={`/leagues/${leagueId}`} className="text-sm text-gray-500 hover:text-gray-700 font-medium flex-shrink-0">
          ← Dashboard
        </Link>
      </div>
    </div>
